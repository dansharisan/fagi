...
export default function Home() {
  ...
  const cryptoFagiHistoricalDates = cryptoFagiData.map(p => (new Date(p[0])).toISOString().split('T')[0]);
  const cryptoFagiHistoricalValues = cryptoFagiData.map(p => Math.round(p[1]));
  const cryptoFagiValue = Math.round(cryptoFagiHistoricalValues[cryptoFagiHistoricalValues.length - 1])
  ...
  return (
    <main className="flex flex-col items-center bg-gray-100 p-8">
      ...
      <p className="mb-4">Welcome to Fear n Greed Index, your comprehensive resource for decoding market sentiment in the US stock and cryptocurrency markets. The Fear n Greed Index is a powerful tool that gauges the emotional state of investors, providing valuable insights into market dynamics. By analyzing a spectrum of factors, this index helps investors understand prevailing sentiments – whether driven by fear or greed.</p>

      {/* Body section */}
      <section id="stock-fagi" className="flex flex-col items-center w-full p-8 mb-8 bg-white rounded-lg shadow-md">
        <h2 className="text-2xl font-bold mb-4">Stock Market</h2>

        <div className="flex w-full space-x-4">
          <button
            className={`flex-grow p-2 ${stockFagiSelectedTab === 0 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
            onClick={() => setStockFagiSelectedTab(0)}
          >
            Overview
          </button>
          <button
            className={`flex-grow p-2 ${stockFagiSelectedTab === 1 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
      ...
      <section id="crypto-fagi" className="flex flex-col items-center w-full p-8 mb-8 bg-white rounded-lg shadow-md">
        <h2 className="text-2xl font-bold mb-4">Crypto Market</h2>

        <div className="flex w-full space-x-4">
          <button
            className={`flex-grow p-2 ${cryptoFagiSelectedTab === 0 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
            onClick={() => setCryptoFagiSelectedTab(0)}
          >
            Overview
          </button>
          <button
            className={`flex-grow p-2 ${cryptoFagiSelectedTab === 1 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
          ...
          <section className="p-4">
            ...
            <GaugeComponent
              ...
              labels={{
                valueLabel: {
                  matchColorWithArc: true,
                  formatTextValue: (value) => explainCryptoFagiValue(value)
                },
                tickLabels: {
                  type: "outer",
                  ticks: [
            ...
            />
            <p className="text-center text-3xl font-semibold" style={{"color": "rgb(173, 172, 171)"}}>{cryptoFagiValue}</p>
          </section>
        )}
              ...
              options={{
                ...
                plugins: {
                  ...
                  tooltip: {
                    callbacks: {
                      label: (tooltipItem: any) => {
                        return `${tooltipItem.raw} (${explainCryptoFagiValue(tooltipItem.raw)})`;
                      },
                    }
                 }
                },
      ...
      <section>
        <h2 className="text-2xl font-bold mb-4">Why Fear and Greed index?</h2>
        <p>In the world of finance, emotions play a significant role in shaping market trends. The Fear and Greed Index quantifies these emotions, offering a unique perspective on investor psychology. Fear often drives markets to undervalue assets, while greed can lead to overvaluation. Recognizing these sentiments can be crucial for making well-informed investment decisions.</p>
      </section>
    </main>
  );
}
